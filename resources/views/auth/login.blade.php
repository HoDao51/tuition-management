...
<body class="flex items-center justify-center min-h-screen bg-[#eff0f3]">
  ...
  <div class="w-full sm:max-w-xl px-6 ">
    <div class="text-center flex flex-col items-center">
      <img src="{{ asset('images/logo_school2.png')}}"
           class="w-[80px] h-[80px] object-contain mb-2">

      <h2 class="bg-gradient-to-r from-orange-400 to-red-700 bg-clip-text 
                text-transparent uppercase text-3xl font-bold text-center">
          Cao đẳng điện lực hoàng gia Scambodia
      </h2>
    </div>

    <div class="mt-10">
      <!-- Bạn để cái này dư, mình chỉ thêm class "hidden" để không hiển thị nữa -->
      <div class="fixed inset-0 bg-white bg-opacity-80 flex items-center justify-center z-50 hidden">
        <div class="animate-spin rounded-full h-16 w-16 border-4 border-gray-300 border-t-red-500"></div>
      </div>

      <form action="{{route('postLogin')}}" method="POST" class="grid grid-flow-row auto-rows-min gap-3 space-y-4 border 
           bg-white/50 backdrop-blur-md rounded-2xl border-gray-200 shadow-xl p-6">
        @csrf
        ...
        <div>
          ...
          <div class="flex-1 relative border border-gray-200 rounded-md ">
            <input id="email" placeholder="Địa chỉ email"
                  class="input input-bordered w-full peer pl-10 pt-3 pb-3 focus:outline-none focus:ring-1 focus:ring-green-400"
