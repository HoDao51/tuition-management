...
<body class="flex items-center justify-center min-h-screen bg-[#eff0f3]">
  ...
  <div class="w-full sm:max-w-xl px-6 ">
    <div class="text-center flex flex-col items-center">
      <img src="{{ asset('images/logo_school2.png')}}" class="w-[80px] h-[80px] object-contain mb-2">
      <h2 class="bg-gradient-to-r from-orange-400 to-red-700 bg-clip-text text-transparent uppercase text-3xl font-bold text-center">
    ...
    <div class="mt-10">
      <form action="{{route('postLogin')}}" method="POST" autocomplete="off"
            class="grid grid-flow-row auto-rows-min gap-3 space-y-4 border bg-white/50 backdrop-blur-md rounded-2xl border-gray-200 shadow-xl p-6">
        @csrf
        <h2 class="text-2xl font-semibold text-[#1f2937]">Đăng nhập</h2>
        <!-- Email -->
        <div>
          <label for="email" class="pt-0 label label-text font-semibold text-[#1f2937]"><span>Email</span></label>
        ...
        <!-- Mật khẩu -->
        <div>
          <label for="password" class="pt-0 label label-text font-semibold text-[#1f2937]"><span>Mật khẩu</span></label>
          <div class="flex-1 relative border border-gray-200 rounded-md">
        ...
        <button type="submit" onclick="showLoader()" class="p-3 w-full text-xl bg-[#ff8e3c] hover:bg-[#CC7130] text-[#1f2937] font-semibold rounded-lg">
          Đăng nhập
        </button>
      </form>
    </div>
