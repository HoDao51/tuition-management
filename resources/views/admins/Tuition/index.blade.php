...
@section('content')
  <div class="mx-auto">
    <div class="flex justify-between">
      <form method="GET" action="{{ route('hocPhi.index') }}" class="flex items-center space-x-2 mb-2">
        <!-- Thanh tìm kiếm -->
            ...
            <td class="px-4 py-2 border text-center pt-5">
              ...
              <form action="{{route('hocPhi.destroy', $item->id)}}" method="POST" class="inline-block">
                ...
                <button type="submit" onclick="showLoader()"
                    class="bg-[#DC2626] text-white text-[16px] font-semibold px-3 py-2 rounded hover:bg-red-800 ml-2 mb-2">
                    Xóa
                </button>
              </form>
              ...
              @if ($item->tinhTrang == 0)
                ...
                <a href="{{route('bienLai.create')}}?hocPhi={{$item->id}}">
                  <button type="button" onclick="showLoader()"
                      class="bg-[#F97316] text-white text-[16px] font-semibold px-3 py-2 rounded hover:bg-[#C55E17]">
                      Thêm biên lai
                  </button>
                </a>
              @endif
