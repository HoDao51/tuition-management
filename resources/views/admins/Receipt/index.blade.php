...
@section('content')
  <div class="mx-auto">
      ...
      <table class="min-w-full border border-gray-200 text-sm text-left">
        <thead class="bg-[#D9D9D9] text-gray-700 uppercase">
          <tr>
            ...
            <th class="px-4 py-2 border text-center">Số tiền đã thu (VNĐ)</th>
    ...
    @foreach ($bienLai as $bienLai)
        <tbody>
          <tr class="hover:bg-gray-50">
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">{{$bienLai->hocPhi->sinhVien->ma_sv}}</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">{{$bienLai->hocPhi->sinhvien->hoTen}}</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">{{$bienLai->hocPhi->hocKy->tenHocKy}}</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">{{$bienLai->hocPhi->hocKy->namHoc->tenNamHoc}}</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center text-red-600 font-semibold">{{number_format($bienLai->soTienThu, 0, ',', '.')}}đ</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">{{$bienLai->ngayThu}}</td>
            <td class="px-4 py-2 border text-[#4B5563] text-[17px] text-center">
               @if ($bienLai->tinhTrang == 0)
            ...
            <td class="px-4 py-2 border text-center pt-5">
                {{-- Nút sửa --}}
                <a href="{{route('bienLai.edit', $bienLai->id)}}">
                    <button type="button"
