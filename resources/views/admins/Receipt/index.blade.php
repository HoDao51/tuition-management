...
@section('content')
  <div class="mx-auto">
    <div class="flex justify-between">
      <form method="GET" action="{{ route('bienLai.index') }}" class="flex items-center space-x-2 mb-2">
          <!-- Thanh tìm kiếm -->
      ...
      <table class="min-w-full border border-gray-200 text-sm text-left">
        <thead class="bg-[#D9D9D9] text-gray-700 uppercase">
          <tr>
            ...
            <th class="px-1 py-2 border w-[160px] text-center">Hành động</th>
          </tr>
        </thead>
        @forelse ($data as $item)
        <tbody>
          <tr class="hover:bg-gray-50">
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center">{{$item->hocPhi->sinhVien->ma_sv}}</td>
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center">{{$item->hocPhi->sinhvien->hoTen}}</td>
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center">{{$item->hocPhi->hocKy->tenHocKy}}</td>
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center">{{$item->hocPhi->hocKy->namHoc->tenNamHoc}}</td>
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center text-red-600 font-semibold">{{number_format($item->soTienThu, 0, ',', '.')}}đ</td>
            <td class="px-1 py-2 border text-[#4B5563] text-[17px] text-center">{{$item->ngayThu}}</td>
