  ...
  <div class="w-full bg-white inline-flex">
    ...
    <div class="flex flex-col items-center space-y-3 p-4 pt-6">
      ...
      <div class="text-center text-[#1f2937]">
        <p class="font-semibold text-[18px]">{{ Auth::user()->name }}</p>
          ...
          <tr>
            ...
            <td class="px-2 py-1">
              @if (Auth::user()->nhanVien->tinhTrang == 0)
                <span class="px-2 py-1 border border-green-400 rounded-full text-green-600 font-semibold">Đang hoạt động</span>
              @else
                <span class="px-2 py-1 border border-red-400 rounded-full text-red-600 font-semibold">Ngưng hoạt động</span>
              @endif
            </td>
          </tr>
        </tbody>
